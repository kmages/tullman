...
from app.howard import answer as howard_answer
...
def _strategy_links_best(max_links:int=2) -> list[dict]:
    ...
    KEYW = ("ai","artificial intelligence","strategy","roadmap","plan","llm","automation","table stakes","table-stakes")
    out, seen = [], set()
    md, links = _safe_finalize(q, md, links)
        ...
        return jsonify({"session_id": j.get("session_id") if isinstance(j, dict) else None,
                        "answer": "Sorry - server error. Please try again.", "sources": []}), 500


    Unified path: allocate sid/prior, delegate to Howard pipeline, finalize safely, return JSON.
    """
    try:
        ...
        # JSON -> GPT-5 (Howard voice) -> Kenifier
        md, links = howard_answer(q, prior)

        # finalizer (non-destructive)
        md, links = _safe_finalize(q, md, links)

        # record turn and return
        hist.append(("user", q))
        hist.append(("assistant", md))
        return jsonify({"session_id": sid, "answer": md, "sources": links})

    except Exception:
        try:
            app.logger.exception("chat_error")
        except Exception:
            pass
        return jsonify({
...
def _safe_finalize(q, md, links):
    try:
        md2, links2 = _finalize_answer(q, md, links)
        if md2:
            md = md2
